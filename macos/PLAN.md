...
★ **只验证了非节点页这一侧**。节点页那一侧没验成：本机 UI **没有切页快捷键**
（`MainView.swift` 里没有 `keyboardShortcut`），`osascript` 发 ⌘2 根本切不了页 ——
早先那组"切到节点页 0.50%"的数字是无效样本（三次其实都停在同一页），已作废。
要验证节点页仍是 1s，需要真人点侧栏，或给 UI 加切页快捷键后再测。
