...
### 已修：空载时 1Hz 无条件重排（-71% 空载 CPU）

`AppState.startBandwidthSampling()` 每秒写 `pollTick` / `bandwidthSamples` /
`displayedUp` / `displayedDown`，这四个都是 `@Observable` 公开属性，**写一次就让 SwiftUI
把整棵视图树重排一遍**。核心没跑、速率恒 0、连接恒空时，界面一个像素都不会变，却仍在每秒重排。

`sample` 的热点 100% 落在 SwiftUI 布局引擎（`LayoutEngineBox.sizeThatFits` /
`StackLayout.placeChildren` / `ViewLayoutEngine.sizeThatFits`），即"纯重排、没有别的活"。

修法：完全静止时整拍跳过，不写任何属性。判据与 Qt 端页脚呼吸圆点那次相同：
**可见运动要值得它的代价**。

★ 判据里**必须带累计量增量**，只看瞬时速率会误判：真机实测（本机回环、经代理拉 1.5 GB）
核心 `/traffic` 的 `up`/`down` **瞬时字段恒为 0**，只有 `upTotal`/`downTotal` 在涨。
第一版判据只信瞬时速率，会在正传着大流量时把这一拍判成静止而跳过，界面就此不再刷新 ——
这是个真回归，靠"改完必须两侧都验"才抓到。现在的判据是
`瞬时 0 && 累计没动 && 无待落快照 && 上一拍也是 0`，少任何一条都宁可多刷一拍。
另外 `bandwidthSamples.last` 在冷启动时为 `nil`，要显式 `?? false`（不能算静止），
否则第一拍就被跳过、图表拿不到起始样本。

回归验证（三段都要对，不能只验省电那一侧）：
静止 0.5~0.9% → 有流量 2.5~2.8%（确实在刷新）→ 停止后回落。
...
布局热点样本数同步从 197 降到 75（-62%），两条证据一致。
★ 注意样本量：先前用 6 次采样看不出差异（方差 0.2%~5.0%），必须 n=20 才能分辨。
