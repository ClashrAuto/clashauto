...
Qt 6.11.1 homebrew arm64 原生、Swift 6.3.3）。都用 `COAST_NO_AUTOSTART=1` 只跑 UI、不起核心、
不动系统代理：

| 指标 | Qt 端 | Swift 端 | 差 |
|---|---|---|---|
| 可执行文件 | 28.0 MB | **6.9 MB** | 4.1× |
| 空载 CPU（瞬时采样 ×6） | 0.0% | 0.0% | 持平 |
| RSS | 221 MB | **131 MB** | 省 90 MB (41%) |
| 线程数 | 20 | **9** | 省 11 |
| 源码行数 | 45783 | 21109 | — |

**结论**：空载 CPU 两边都到 0，UI 框架在闲置时都不烧 CPU；Swift 端的实际优势在**常驻内存
（省 41%）、线程数（省 11 条）和二进制体积（1/4）**。

### 两个量法上的坑（都实际踩到过，结论一度反了）

1. **别跨机比。** 先前把 Qt 端跑在 .34（Intel i7-7567U / 4 核 / load 1.5）、Swift 端跑在本机
   （arm64 / 8 核），得出的任何差异都是机器差异。本机装上 Qt 6.11.1 后重测才有意义。
2. **`ps -o %cpu` 是进程生命周期均值，不是瞬时值。** 跑得久的进程会被稀释：同一次测量里
   Qt 显示 1.15%（offscreen 短跑）、真渲染长跑却显示 0.14%，而 Swift 短跑显示 3.30% ——
   全是采样窗口造成的假象。要瞬时值必须用 `top -l 1 -pid <pid> -stats cpu`，
   两边一致后都是 0.0%。
