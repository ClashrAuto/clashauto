...
**下一步该往哪走**（本轮没做完，留给后续）：
- 逐个属性省是死路（已验证）。要动就得**缩小重排波及面**：
  状态页两张 `TrafficCard` 各自在 body 里做 `state.bandwidthSamples.map(\.up)` /
  `map(\.down)` —— 每次重排都对整个样本窗口做一次全量映射，两张卡两次。
  可预先在 `AppState` 里维护两个已拆分好的数组，视图直接取，省掉每次 map。
- `pollTick` 被状态页、设备页、设备详情**同时**读，+1 一次波及面最大，
  可考虑按页拆成各自的节拍，避免一个信号唤醒所有页。
- 已做且保留：`displayedUp/Down` 改成"变了才写"。**单独看没降 CPU**（中位仍 1.80%），
  但逻辑上正确、无副作用，作为上面那条路的一部分留着。
