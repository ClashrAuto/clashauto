...
**下一步该往哪走**（本轮仍未解决）：
- 已知：把这一拍的活**全部搬走**能省 1.2%，但逐项省不下来。
  可行方向是**把整拍的多次 `@Observable` 写入合并成一次通知**
  （例如攒成一个值类型快照、一次性赋给单个被观察属性），让 SwiftUI 只收到一次变更、
  只重排一次，而不是一拍之内被唤醒多次。
- 未测项：`applyPendingSnapshot()` 是循环体里唯一还没单独量过的（本轮试了但编译失败、
  跑的是旧二进制，数据作废，不作数）。下轮补上。
