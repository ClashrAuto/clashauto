...
**下一步（仍未解决）**：
- 成本 ∝ 窗口面积 ⇒ 要降就得让**每拍不重排整个可见区域**。方向是把每拍会变的东西
  （曲线）从主布局树里**彻底摘出去**，让它的更新不进入 SwiftUI 的 layout pass ——
  代码里 `SlidingCurve` 已经把折线交给 CoreAnimation 了，但整张图仍在每拍重算 `body`。
  下一步应确认：`body` 重算是否必然带来 layout pass？若是，则要让 `TrafficCard` 的
  `samples`/`tick` 不再是 `body` 的输入（改由内部 `NSView`/CALayer 自持）。
- 对照参考：Qt 端同样是每秒刷新，开窗只要 0.60% —— 它的 `BandwidthChart` 是 QML `Canvas`，
  重绘完全不参与布局。这是 3 倍差距的结构性来源。
