...
// ★★ 这个池装的**不只是"并发连接"**——主动关闭方留下的 TIME_WAIT pcb 也占坑，而它们才是大头。
//   本网关几乎总是主动关闭方（HTTP `Connection: close` 之类由服务器先发 FIN，我们随即 tcp_close），
//   所以真正的量纲是：
//       需要的池容量 ≈ 并发活连接数 + **新建连接速率 × TIME_WAIT 停留时长**
//   老版本只按前一项取 2048，把后一项整个漏了：真机 2500 conn/s × 120s(默认 TCP_MSL) = 30 万条，
//   于是池子在**不到 1 秒**内被 TIME_WAIT 塞满并**永远保持满员**（实测 pcb=2048/2048、tw1984，
//   97% 是 TIME_WAIT），此后每一条新连接都要先走 tcp_alloc 的降级链杀掉一条最老的 TIME_WAIT ——
//   那是**每次全表扫描**（twScan 实测 5.6M 次节点访问/秒）。TCP_MSL 已经从 60s 降到 500ms
//   （见「TCP 调参」那一段，那是止血的主刀），这里把容量也补齐，让池子**不再常态满员**：
//       4096 / 1.2s ≈ 3400 conn/s 才会重新踩到降级链
//   ——覆盖实测峰值（3200 conn/s）且对家用场景（几十~几百 conn/s）有两个数量级余量。
// 为什么在意「常态满员」而不只是在意够不够用：满员时 tcp_alloc 的降级链是
//   杀最老 TIME-WAIT → 杀 LAST_ACK → 杀 CLOSING → **杀一条活连接** → 才丢 SYN，全程无日志。
// 这条链在本负载下停在第一步（无害），但把系统长期钉在「靠杀东西才能分配」的临界点上，
// 只要 TIME_WAIT 一时不够杀就会直接开始伤活连接。诊断栏 twKill/laKill/prioKill 就是看这个的。
//     4096 × 304 = 1,245,184 B（≈1.19 MiB，v6 双栈后 tcp_pcb ≈304 B；相对上一版 +0.66 MiB）
// 不取更大的理由不是内存，是 tcp_active_pcbs 是**单链表**：tcp_slowtmr(200ms) 每次全表走一遍
// （2048 项 ≈ 0.1 ms，可忽略），tcp_input 查表虽是 O(n) 但命中后会把该 pcb 移到表头(MRU)，
// 热流基本 O(1)。**更关键的一条**：tcp_input 对**每个 SYN** 都要把整条 tcp_tw_pcbs 走一遍
// （见 tcp_in.c 的 TIME-WAIT 查找循环，无 MRU、无提前退出），所以池子越大、TIME_WAIT 表越长，
// 建连就越贵 —— 真正压住这项开销的是 TCP_MSL（表长 = 速率 × 停留时长），不是池容量。
//     2048 × 272 = 557,056 B（≈544 KiB） —— 旧值，留作对照
...
// ★★ TIME_WAIT 停留时长 = 2 × TCP_MSL。**不给这个值，整个网关会在持续新建连接下彻底停止服务。**
//
// 真机实证（并发 64、~2500 conn/s、被劫持设备打 netns 靶机；诊断见 gateway-diag.log 的
// pcbst=/synHitTw= 两栏）：lwIP 默认 TCP_MSL=60000 ⇒ TIME_WAIT 停 **120 秒**。本网关是这条
// 连接的**主动关闭方**（靶服务器 `Connection: close`，FIN 先到 → 我们 tcp_close），所以**每条**
// 连接结束都留一个 TIME_WAIT pcb。于是：
//   · 稳态：`pcb=2048/2048`、`pcbst=…tw1984…` —— 池子 **97% 是 TIME_WAIT**，每条新连接都要靠
//     tcp_alloc 的降级链杀掉一条最老的 TIME_WAIT 才拿得到 pcb（twKill≈2800/s），而
//     tcp_kill_timewait 是**每次全表扫描**：twScan 实测 **5.6M 次链表节点访问/秒**。
//     另外 tcp_input 对每个 SYN 还要把同样长的 tw 链表再走一遍（那一遍没有计数器）。
//   · 停摆：被劫持设备的临时端口范围只有 28232 个，2500 conn/s 下约 11 秒转一圈。压力一停，
//     TIME_WAIT 表**不再被回收**（没有新连接就不会调 tcp_kill_timewait），2048 条原地冻结 120 秒；
//     下一轮压力起来时设备复用到落在这张表里的端口 → SYN 命中 TIME_WAIT 的四元组 →
//     tcp_timewait_input 因序号在窗外而**静默丢弃**（不 RST、不回调、无任何日志）→ 设备只能
//     RTO 重传**同一个四元组** → 再次命中 → 死循环。实测日志里就是
//     `tcpAcc=0 tw2048 synHitTw=64`（64 = 客户端并发数，一条不落全卡住），
//     且**因为没有新连接、TIME_WAIT 表更不会被回收 —— 停摆本身维持着造成停摆的状态**，
//     这就是那个「压力一停就好、一压就死」的活锁。实测一次停摆持续 10~11 秒。
//
// 取 500ms（⇒ TIME_WAIT 实际 1.0~1.2s，见下面的取整）：
//   · lwIP 的 60s 是**广域网**常数（RFC 793 的 MSL 建议值）。本移植这条 TCP 的对端是**同一个
//     局域网里的设备**，RTT 0.3~30ms，报文在网络里最多存活几毫秒 —— 60s 高出五个数量级。
//   · TIME_WAIT 在这里只剩一件事有意义：设备侧处于 LAST_ACK、我们的最后一个 ACK 丢了时，
//     它会按 RTO（Linux 下限 200ms）重传 FIN，我们得还在、能补一个 ACK。1.2s 覆盖前两次重传。
//     再丢的话设备收到的是 RST —— 而它此刻**已经把全部数据交给应用了**，纯属无害收尾。
//   · 另一件（防旧连接的游荡重复段被新连接误收）要求设备在 1.2s 内复用同一个源端口，
//     28232 个端口的轮转下等价于 >23000 conn/s 才可能发生，且还要过序号窗口检查。
//   ★ 实际停留时长按 tcp_slowtmr 的拍数取整：判据是
//     `tcp_ticks - pcb->tmr > 2*TCP_MSL/TCP_SLOW_INTERVAL`，TCP_SLOW_INTERVAL = 2×TCP_TMR_INTERVAL
//     = 200ms ⇒ 阈值 5 拍 ⇒ 第 6 拍才回收 ⇒ 1.0~1.2s。**改 TCP_TMR_INTERVAL 会连带改这个值。**
//   ★ 与 MEMP_NUM_TCP_PCB 是一对：池子够不够用 = 连接**速率** × TIME_WAIT 停留时长（见那一段）。
#define TCP_MSL                         500
