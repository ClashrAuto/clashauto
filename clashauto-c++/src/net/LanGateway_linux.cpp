...
void GatewayWorker::applyCoastCoreLocal()
{
    ...
    if (m_coastCore && m_pcfgStore) {
        ...
        factory->setRouter([store, rules](const QString &dstHost, const QString &user) -> QString {
            Q_UNUSED(user);
            ...
            case ProxyConfig::Mode::Rule: {
                ...
                const QString node = (mo.needsResolve || mo.target.isEmpty())
                                         ? QString()
                                         : cfg->resolveTarget(mo.target);
                // 诊断（COAST_GATEWAY_DEBUG，默认不打印）：Rule 模式每条连接的判定链路
                //（命中的 target → 解析出的节点），排查「为什么回退/为什么走了这个节点」就靠它。
                if (gwDbgOn()) {
                    qInfo().noquote() << "[CCROUTE] host=" << (hostForRule.isEmpty() ? dstHost : hostForRule)
                                      << "needsResolve=" << mo.needsResolve
                                      << "target=" << mo.target << "node=" << node
