...
#include "core/CoreRouter.h"        // 分流判定的唯一实现（与本机入站共用）
...
void GatewayWorker::applyCoastCoreLocal()
{
    ...
    if (m_coastCore && m_pcfgStore) {
        ...
        // 分流判定本身已提到 core/CoreRouter（本机入站 MixedInbound 要用同一份实现——
        // 复制一份就会开始漂移，「本机走了 A 节点、设备走了 B 节点」这类问题极难查）。
        factory->setRouter(coastcore::makeRouter(store, rules, gwDbgOn()));
