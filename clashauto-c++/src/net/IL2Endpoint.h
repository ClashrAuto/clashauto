...
public:
    ...
    // 把 send() 攒下来的帧**立刻**交给驱动。默认空实现 = 该后端的 send() 本来就是立即发。
    //
    // 为什么会有「攒」这回事（Windows/Npcap 专属，见 L2Endpoint_win.cpp 的发方一节）：
    //   pcap_sendpacket 一次只提交**一个**包，且要等 NDIS 发送完成才返回。真机 6000 个诊断窗口
    //   的回归结果是「每发一帧 ≈ 306 µs 的工作线程墙钟」——用户态那点活（一次 QByteArray 分配 +
    //   1500 B memcpy）只有 0.3 µs，差三个数量级，剩下的全在驱动里等空口。逐帧提交还有个致命
    //   副作用：WiFi 驱动每次只拿到一个包，**没有机会做 A-MPDU 聚合**，于是每帧都付一次完整的
    //   DIFS+退避+PLCP+SIFS+ACK。本机自己走 mihomo 的 TUN 完全没有这一层（Windows 协议栈交给
    //   WiFi 驱动的是成串的 NBL），这正是「同一台机器、本机 484 KB/s vs 被代理设备 142 KB/s」的
    //   机制。于是 Windows 后端改成攒一批一次提交（pcap_sendqueue_transmit，驱动侧一次性投递、
    //   只等一次完成）。
    //
    // 调用契约（**漏调就是设备侧卡住**，加新的出帧路径时务必想清楚谁来 flush）：
    //   · 一批帧产生完就要 flush，绝不能等下一次「碰巧有人 flush」。现有的三个点是
    //     ① 收帧排空的末尾（一次唤醒里由 lwIP 顺带打出来的所有帧）
    //     ② NetStack::pumpToLwip 末尾（下行数据触发的那一串段）
    //     ③ lwIP 定时器泵每拍（重传/延迟 ACK/ARP 投毒这类不挂在上面两条路上的帧）
    //     ③ 同时是**兜底**：任何漏网的出帧路径最迟 25 ms 也会被发出去。
    //   · 实现必须允许在队列为空时被随便调（空调用只做一次判空）。
