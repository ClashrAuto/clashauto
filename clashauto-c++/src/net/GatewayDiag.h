...
public:
    ...
    struct Counters {
        ...
        qint64 synAckTx;
        // 走到 NetStack::inputFrame 的**纯 SYN** 帧数（含设备的重传）。
        // ★ 与设备侧 tcpdump 抓到的 SYN 数直接对照，这是最后一个差值：
        //   相等 → 帧都进来了，问题在栈之后（但栈已被三条 Rust 用例排干净）；
        //   我们少 → 帧在空口丢了或 Npcap 没交付（注意不是"丢弃"，rxdrop 已实测为 0）。
        //   真机现状：单流完美 460 KB/s，8 并发只有 2 条成功，而 synackTx=tcpAcc=8、
        //   synSlow=0、rxdrop=0 —— 失败流的 SYN 看起来根本没被抓到。
        qint64 synRxIn;
