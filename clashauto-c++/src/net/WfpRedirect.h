...
//      （这台 Windows 是虚拟机，网卡是模拟的 Intel PRO/1000 MT；多数 hypervisor 的虚拟交换机
//       默认不放行 VM 发出的"非自身 MAC/异常"二层帧，正撞这条。）
//      → 待办收窄为二选一：**在物理 Windows 机上复跑**，或**放开宿主虚拟交换机的
//        MAC-spoof / promiscuous 策略**后再测。在此之前，Windows 网关的功能性无法定性，
//        任何"Windows 网关不通"的结论都不能归到我们的代码上。
//
