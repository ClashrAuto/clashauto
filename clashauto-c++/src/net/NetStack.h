...
//
//     ★★ **万兆的生产口径因此是 ~4.8 核**（10 Gbps ÷ 2.1 Gbps/核），不是上面那个 0.3~0.6 核。
//        本机只有 4 核 —— 换言之**这台 Pi 做不到加密万兆**，瓶颈是加解密而不是数据面。
//        要上万兆要么靠更多核/更快的 AES 指令，要么让流量尽量走 DIRECT（splice 那条快路）。
//        TPROXY 相对 lwIP 的价值不变（数据面便宜一个量级），但别再用 DIRECT 的数字做产品承诺。
//
