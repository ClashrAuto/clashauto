...
//   → **TPROXY 省 3.7 倍 CPU。**
//
//   ── 2026-08-04：lwIP 还有一笔 CPU 之外的账 —— **每条 TCP 连接白送一个上游 RTT** ──────
//     上面只比了吞吐/CPU，漏了**建连延迟**。在 Windows 网关（lwIP）与 Pi 网关（TPROXY）上
//     同时段、同一台靶机口径实测（curl 的 time_connect，即 SYN → SYN-ACK）：
//         目的地                    Windows lwIP      Pi TPROXY
//         Cloudflare（走代理节点）   142~178 ms        0.4~0.5 ms
//         百度（命中 DIRECT 规则）    9~11 ms          1.1 ms
//     靶机侧抓包直接量到 SYN → SYN-ACK = 143/175/178 ms，与 curl 一致。
//     ★ 判据在第二行：换成上游只有 ~9ms 的国内目标，建连也跟着掉到 ~9ms ——
//       **建连时间 = f(上游 RTT)**，说明设备的三次握手被压到「上游连通之后」才完成。
//       TPROXY 没这笔开销：SYN 由内核当场交给核心的 tproxy listener，握手就地完成（~1ms），
//       上游是之后并行拨的。
//     ★ 代价有多大：一个出海 RTT ≈ 160ms，网页几十条连接就是**几秒**的额外首屏时间，
//       而且这笔钱**吞吐测不出来**（iperf3 单条长连接只付一次）。先前"lwIP 只是费 CPU"
//       的印象是被测法选择性掩盖的。
//     ★ 这不是调参能解决的：收包已经是 to_ms=1 + setmintocopy(1)，泵是 25ms，都不在这个量级。
//       要修就是把「本地先完成握手、上游并行拨号」做进 NetStack —— 而方向本来就是**去掉 lwIP**
//       （Windows 走自写 WFP 驱动、macOS 走 pf/BPF），所以这条记在这里当**又一条 lwIP 必须退场
//       的量化理由**，不在 lwIP 里补丁。
//
