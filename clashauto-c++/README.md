...
- 单实例（`QLocalServer`/`QLocalSocket` 命名管道）：多开时后启实例把参数转发给已运行实例并退出、前置窗口；CLI 子命令不受限。
- 协议导入订阅（`clash-auto://`）：启动时注册协议到 Windows 注册表（HKCU，免管理员）；点击 `clash-auto://import?url=<编码地址>&name=<名>&type=<sub|clash>`（或 `install-config?url=...`）即转发给运行中实例，自动加订阅并拉取节点。
- 节点切换通知（`note`，设置「切换通知」可关）：自动/手动换节点时弹系统通知（跳过启动首次）。
- 关于页「导出节点」按钮：把状态页当前分组节点（名称+延迟）导出为 JSON 到「下载」目录。
- 关于页「打开面板」按钮：打开 yacd 在线面板并带上本机 API host/port。
...
- 核心以 `-d userDir` 启动（家目录固定为用户配置目录）：GeoIP/缓存路径由构造保证一致；首启自动从 bundle 种子 `Country.mmdb`。（Windows TUN 不需要我们部署 `wintun.dll` —— 核心自带并从内存加载，见 `CoreController::startCore` 的注释。）
...
```text
C:\Users\Administrator\AppData\Roaming\Coast\config\full.yaml
```

可用原项目内置 Clash 核心验证：

```powershell
G:\clashauto\Clashr-Auto\command\clash\clash-windows-amd64.exe -t -f "C:\Users\Administrator\AppData\Roaming\Coast\config\full.yaml"
```
