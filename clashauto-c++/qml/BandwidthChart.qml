...
//
// ── 怎么验证上面这条真的成立（改完这个文件请务必跑一遍）──────────────────────────
// 光看代码看不出来：Canvas 只要**尺寸**每帧变哪怕 1 像素，就会整块重新光栅化并重传纹理，
// 而位置(x)变多少都不会。两者在代码里长得一模一样，只有场景图的计时日志能分开。
//
//   Windows:  set QSG_RENDER_TIMING=1 & set QT_FORCE_STDERR_LOGGING=1 & Coast.exe > qsg.log 2>&1
//   Linux/mac: QSG_RENDER_TIMING=1 ./coast 2> qsg.log
//   （Windows 上 QT_FORCE_STDERR_LOGGING 不能省：GUI 子系统程序的 qDebug 默认走
//     OutputDebugString，不落 stderr，日志文件会是 0 字节。）
//
// 然后数两件事：
//   grep -c "frame rendered" qsg.log          # 出帧数
//   grep -oE "\([0-9]+x[0-9]+\)" qsg.log | sort | uniq -c    # 纹理上传的尺寸分布
//
// **健康**：上传次数应该 ≈ push 频率（1/s 量级），且尺寸只有一种。
// **坏掉了**：上传次数 ≥ 出帧数，且尺寸在相邻两个值之间交替。真机上见过的坏样子——35 秒内
//   3814 次上传（109 次/秒，是出帧率 24.5fps 的 4 倍多），尺寸严格在 347x170 / 348x170 之间
//   各 1907 次对半分。那就是某个元素的宽度在 1 像素上来回抖，每帧都在重光栅化 + 重传。
// 这也是 spacing 用 Math.ceil 取整、canvas 宽度写成 `整数 spacing × 整数点数` 的原因：
// 让宽度**恒为整数**，滚动只改 x。别把宽度改成含 root.width 的小数表达式。
//
...
//   而曲线贴着底边的时候它就是一条平线，**平移它在视觉上没有任何区别**。
//   判据按**像素**算而不是按字节：峰值折算成控件里的高度，低于 0.25px（2x 屏上半个
//   设备像素）就停（见 hasMotion）。按字节判是不够的——量程下限 128KB/s，后台几百 B/s
//   的涓流就能让它一直「在动」，而那条线的峰值还不到半个像素。观感一模一样，
//   闲置时整窗不再出帧。（Swift 端 SlidingCurve.applySlide 是同一条判据。）
Item {
    id: root
    ...
    // 窗口里还有没有「看得出在动」的东西。见文件顶部那段 ★：曲线贴底时它是条平线，
    // 滑它等于让整窗白白每帧重画。
    //
    // ★ 判据按**像素高度**算，不按字节数。原先是「有没有样本 > 1 B/s」——
    //   那是个与量程无关的绝对值，量程最小也有 128KB/s，于是后台常年几百 B/s 的涓流
    //   （任何一个在轮询的程序都会产生）就足以让它一直判「在动」，而那条曲线的峰值
    //   高度还不到半个像素。结果是「看着完全静止、整窗却一直按步进帧率合成」。
    //   改成和 Swift 端 `SlidingCurve.applySlide` 同一条判据：峰值折算成控件里的像素
    //   高度，低于 0.25px（2x 屏上半个设备像素）就是**不可能看见**的运动，停掉。
    //   注意这只会停掉证明不可见的运动，看得见的滚动一帧都不少 —— 流畅度不受影响。
    readonly property real kIdlePx: 0.25
    function hasMotion() {
        if (maxScale <= 0 || height <= 0)
            return false;
        var peak = 0;
        for (var i = 0; i < pointers.length; ++i)
            if (pointers[i] > peak)
                peak = pointers[i];
        var frac = peak / maxScale;
        if (frac > 1)
            frac = 1;
        return height * headroom * frac >= kIdlePx;
    }
