    ...
    RowLayout {
        ...
        Item {
            ...
            ColumnLayout {
                ...
                // logo：无背景的纯地球图标（#4898F8）+ 右下状态角标。窗口拖动由上方窗口级背景 handler 负责。
                Item {
                    ...
                    Item {
                        ...
                        // 状态角标（右下，压在地球右下角）：白圆角 + 字母——增强 T / 网页 W / 核心开 C / 核心关 N。
                        Rectangle {
                            width: 26
                            height: 26
                            radius: 7
                ...
                // 导航（与 StackLayout 索引 1:1 对齐）。navIcons 与 model 顺序一一对应（Remix 码点）：
                // 状态 dashboard / 节点 global / 设备 device / 订阅 rss /
                // 设置 settings-3 / 日志 file-list-3 / 关于 information。
                Repeater {
                    id: navRepeater
                    readonly property var navIcons: ["\uEC14", "\uEDCF", "\uEC2E", "\uF09F", "\uF0E6", "\uECEF", "\uEE59"]
                    model: [qsTr("状态"), qsTr("节点"), qsTr("设备"), qsTr("订阅"), qsTr("设置"), qsTr("日志"), qsTr("关于")]
                    delegate: NavButton {
                        Layout.fillWidth: true // 铺满侧栏宽、右缘紧贴内容卡
                        Layout.leftMargin: 20  // 按钮左侧留 20px 间距
                        Layout.topMargin: index === 0 ? 0 : 5
                        label: modelData
                        icon: navRepeater.navIcons[index]
