...
## ⚠️ 关于"剩下那 4.5 倍差距"——**没测出来，且测量本身已不可信**

fd 修好之后想继续挖，结果**没能收敛**，如实记录以免下次重蹈：

- 同一套负载（并发 10 / 12s），一小时前是 **4485 成功**，之后反复在 **0~10 成功**之间；
- 但**单条 curl 经网关始终正常**（`200`，1.7ms），劫持也确认生效（设备侧网关 MAC = 树莓派 OUI）；
- 埋点显示网关在建连（`tcpActive=383`、`connMs` 有 384 条），但客户端等不到；
- 重启网关（清空全部 lwIP PCB）**不复现**；把目标服务端改动回退**也不复现**。

我犯的一个方法错误：为了验证"目标 backlog 只有 5"这个假设，**一次改了两个变量**
（backlog 5→4096 **和** HTTP/1.0→1.1）。回退 HTTP 版本后仍是 0，说明两者都不是原因，
但这一步本身违反了对照原则，浪费了几轮。

**结论：目前这台测试台在高并发下有未识别的漂移变量，任何"网关比直连慢 N 倍"的数字都不该采信。**
要继续这条线，下次应当：
1. 先建立**可重复的基线**——同一配置连跑三次，差异 <10% 才开始比较；
2. 负载客户端换到**第二台设备**，把"客户端自身瓶颈"这一类彻底排除（本轮查过 TIME_WAIT 只有 2，
   但不代表没有别的客户端侧限制）；
3. 目标服务器换成 C 写的极简 echo/HTTP（python 的 GIL 与线程模型本身就是变量）。

**已经落地的三个修复不受此影响**——它们各自有独立、可复现的证据（崩溃栈、fd 顶死曲线、
断言消息从无到有），与这段没测出来的差距无关。
