...
## 轻载延迟：进程内引擎比回退 mihomo 快 36%（2026-07-31）

同一条路、同一台设备、同一个靶机（netns `10.99.0.2`），**只换 `coastcore` 开关**，各 8 次请求：

| | 连上 | 首字节 | 总计 |
|---|---|---|---|
| `coastcore: false`（出站回退 mihomo） | 5.0 ms | 22.2 ms | **22.3 ms** |
| `coastcore: true`（进程内出站） | 4.4 ms | 14.3 ms | **14.3 ms** |

`cc=8/0/0/0/0/0`（全部进程内、零回退）、`connMs` 绝大多数落在 **<1ms** 桶 —— 确认走的是进程内，
且**进程内建连本身不到 1 毫秒**。⇒ **「替换 mihomo」在延迟上是净收益，不是持平。**

### 剩下的时间在哪（已排除的三项）

双侧同时抓包（设备侧 eth0 + 靶机侧 veth-h）把一次请求摊开后，可以逐段定位：

- **出站拨号**：进程内 `connectTo→established` 相对裸 `QTcpSocket` 只多 **0.033 ms**
  （`tools/inbound_e2e` 实测，300 次）—— 已排除；
- **数据面泵**：`late=0 maxLagMs=0`、零丢弃零积压、CPU 单核 15% —— 已排除；
- **泵的 25 ms 节拍**：`flushNicTx` 那条「最迟 25 ms 兜底」是 **Windows 专有**的（Linux 的
  `flushTx()` 是空实现，发送即时），对派上的网关不成立 —— 已排除。

剩下的两段都在 **lwIP 这一侧**：设备 SYN → SYN-ACK 约 **4.4 ms**（本该是微秒级），
以及请求转发/应答回送约 **10 ms**。对照内核转发全程只要 **0.5 ms**。

### 真凶：TPACKET_v3 的块超时（已修，延迟 15.7×）

**不在 lwIP，在收包那一层。** `L2Endpoint_linux.cpp` 里 `tp_retire_blk_tov` 那段注释早就预言了：
V3 只有「块填满」或「块开启超过 tov」才把整块交给用户态，而 `tov = 1ms` 会被内核
`msecs_to_jiffies` **向上取整到 1 个 jiffy** —— 树莓派 OS / Debian 的 `CONFIG_HZ=250` ⇒ **4 ms**
（已在派上核对：`CONFIG_HZ=250`，内核 6.12）。设备的**每个**帧（SYN / ACK / 请求 / FIN）都要等这一下，
一次 HTTP 请求跨好几个来回，于是被放大成十几毫秒。

当时把它判为「相对整条链路可接受」。**实测不是**——只换这一个变量：

| | 连上 | 单请求总计 | 饱和吞吐（3×10s） | `rxdrop` |
|---|---|---|---|---|
| 开环（TPACKET_v3） | 2.53 ms | **11.00 ms** | 2599/2572/2441 = 2537/s | 0 |
| 关环（recvmmsg，逐帧就绪） | 0.28 ms | **0.70 ms** | 2863/2871/2915 = **2883/s** | 149 |

**延迟 15.7 倍改善，而且吞吐还高 14%** —— 环换来的批处理收益抵不过它自己的唤醒延迟。
关环后 0.70 ms 与**内核转发的 0.5 ms 基本持平**，也就是说网关这条路已经没有明显的额外开销了。

⇒ 已把环改成**默认关闭**（要用得显式 `COAST_L2_RING=1`）。代价说清楚：突发吸收变差
（`rxdrop` 0 → 149；两种配置失败率都是 0%，TCP 重传兜住了）。环那 1 MiB 缓冲只在
「用户态排空跟不上」时才有价值，而实测我们跟得上（`pumpLag` 几乎全零）。
**真正的最优解是 TPACKET_V2**：逐帧就绪、没有 tov，同时保留 mmap 零拷贝与环缓冲——留待后续。

**教训**：这个风险在代码注释里写得清清楚楚，还给了替代方案，但当时是**估计**「可接受」而没有量。
一个被写下来的已知取舍，如果从没被测过，就跟没写一样。
