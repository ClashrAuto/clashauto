...
| 环节 | 实测/推算上限 | 是不是今天的瓶颈 |
|---|---|---|
| **泵周期量化（25 ms）** | 📏 **41.9 Mb/s / 连接** | ★ **是。且远比其它项紧** |
| Npcap TX（修完 loopback） | 📏 572 Mb/s | 否 —— 现在够不着它 |
| smoltcp CPU | 📏 0.21 核/Gbps | 否 |
| 核心中继（DIRECT） | 📏 0.38 核/Gbps | 否 |

📏 = 实测。泵量化那一行来自本次新增的 `measure_bytes_per_poll_cycle`（`rust/coaststack/src/e2e.rs`），
可复现：`cargo test --release measure_bytes_per_poll -- --nocapture`。
...
```
[QUANT] 灌入 292000 B，单个 poll 周期交出 131072 B → 按生产 25ms 泵折算 41.9 Mb/s/连接
```

131072 B 正好是 `RX_BUF`（128 KiB，`engine.rs:39`）—— 一个周期最多吃下一个接收窗口，
和窗口/RTT 的经典公式一致，只不过这里的「RTT」是**我们自己的泵周期**，不是链路 RTT。

**而且 41.9 Mb/s 还是乐观值**：窗口更新 ACK 要等下一拍才发出去，真实设备在收到它之前不会
继续发 —— 实际的往返是「数据进(1 拍) + 窗口出(1 拍)」，单连接更接近 **~21 Mb/s**。
...
**验证判据**（别只看能不能跑）：`measure_bytes_per_poll_cycle` 那条测量的语义会变 ——
改完之后该测的是「单位时间的 poll 次数」和端到端吞吐，而不是「每拍多少字节」。
