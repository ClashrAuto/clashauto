...
| 本机应用流量（系统代理） | ✅ **端口换位真机验通**（系统代理那一跳仍需桌面会话） | 见下节「端口换位」：进程内入站直接占走 **7890**（系统代理指向的口，无需改指向），mihomo 挪到 7891 只作回退出口 |
...
### 端口换位：本机那条路也进了进程内（2026-07-31，Pi .91 真机）

以前本机入站另起一个 **7891**，要靠改系统代理指向才能用上——那一步只有真实桌面会话验得了，于是
「本机流量也替换掉」一直卡在这。现在改成**换位**：**进程内入站占走 `mixedPort`（默认 7890，
系统代理本来就指着它，指向完全不用动）**，mihomo 的 `mixed-port` 挪到 **`mixedPort+1`**，
只作为进程内不支持的节点（grpc / salamander / tuic…）的**回退出口**。两个端口都由 `config.yaml`
的 `port:` 推导（写死的 `kLocalInboundPort` 已退休）。设置页一个开关「全部切换到进程内」一次切换。

**顺序就是安全性本身**（最坏结果是「系统代理指着的端口没人监听」= 整机断网）：
① 先试绑 7890 → ② 绑不上且核心在跑，才把核心挪到 7891 并**确认新口起来** → ③ 再抢绑 7890 →
④ 抢不到就**回滚**核心、拒绝切换。切回同理：交还前先确认有人能接，接不回就把端口抢回来继续服务。

**真机证据（Pi .91，Debian aarch64，用户真实订阅 56 节点：27 vless-grpc / 19 hy2 / 10 tuic）**：

| # | 场景 | 结果 |
|---|---|---|
| ① | 切换后「确实走进程内」（**严格模式 + Direct**，严格 = 没有回退这条路，成功即证明是我们自己拨的） | `curl -x http://127.0.0.1:7890` **5/5 = 404**，`cc` 的进程内栏 **0→5 精确对齐**，严格拒绝栏不动 |
| ② | 回退出口跟着挪（**非严格**，选 reality+grpc 的 HK-1，进程内不支持） | `https://www.google.com` **3/3 = 200**，`cc` 缺协议栏 **0→3**（=全部走回退）；**反向对照：杀掉核心后同一请求 000「Connection refused」** |
| ③a | 7890 被别的程序占住 + 核心没跑 → 切换 | **拒绝切换**，原持有者仍在服务（`http=418`），20ms 探针 398 次**零断口**，配置未留半截 |
| ③b | 核心在跑，抢占者在核心让位瞬间夺走 7890 | **回滚 + 拒绝切换**，7890 仍由抢占者服务，20ms 探针 596 次**零断口** |
| ④ | 切回 | 核心回到 7890（**PID 不变 = 没重启核心**）、7891 无人、`curl` 404、`full.yaml` 的 `mixed-port` 与 `config.yaml` 全部还原 |
| ⑤ | 切回时**核心不可用**（杀进程 + 移走二进制） | **拒绝交还端口**、保持进程内入站继续服务并报原因；探针零断口；恢复二进制后再切一次即正常还原 |

**两个由真机数据逼出来的实现细节**：
- **热重载必须 `?force=true`**：`mixed-port` 在 general 段里，mihomo 默认热重载**不看它** —— 第一版
  实测「重载后 7891 起不来」，只能重启核心兜底（代价是断掉全部在途连接）。加 force 后核心
  **不重启**就换绑（PID 不变已复核两次）。
- **换手窗口只在「切回」那一侧，约 1.1 s**（20ms 探针定位到 off 那一次；开的方向**零断口**，
  因为抢绑重试是 20ms 粒度）。这 1.1 s 是**核心自己 reload 到重新 bind** 的时间：我们已把
  「生成 full.yaml + `mihomo -t` 校验」提前到入站还在服务时做完（`CoreController::prewarmConfig`），
  剩下的在核心侧。终态不变量（永远有人服务 7890）不受影响。

**仍未验**：系统代理那一跳（Windows WinINET / macOS SCPreferences / Linux gsettings）在真实桌面
会话里的表现 —— 换位后 `proxyPort()` 前后都返回 `mixedPort`，代码上就不需要改指向，但**没在
真机点过开关**。Windows 侧只做了编译 + 逻辑自检。
