...
fn main() {
    ...
    if args.len() != 4 {
        eprintln!("usage: smoltcp2socks_v6 <tap> <v6ip> <prefix>");
        std::process::exit(2);
    }
    let (tap_name, ip_str, prefix_str) = (&args[1], &args[2], &args[3]);
    ...
    let prefix: u8 = prefix_str.parse().unwrap_or(64);
    ...
    if let IpAddress::Ipv6(self_v6) = ip {
        iface.routes_mut().add_default_ipv6_route(self_v6).unwrap();
    } else { eprintln!("v6_main 需要 IPv6 地址"); std::process::exit(2); }
    ...
    eprintln!("smoltcp2socks_v6 up on (IPv6) {tap_name} ip={ip_str} wnd={WND} ports={PORTS:?} \
