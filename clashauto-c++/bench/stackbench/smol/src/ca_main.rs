...
use catchall::{CatchAll, PortMap, FIXED_PORT};
...
fn main() {
    ...
    if args.len() != 4 { eprintln!("usage: smoltcp2socks_ca <tap> <ip> <mask>"); std::process::exit(2); }
    let (tap_name, ip_str, mask_str) = (&args[1], &args[2], &args[3]);

    let base = TunTapInterface::new(tap_name, Medium::Ethernet)
        .unwrap_or_else(|e| { eprintln!("tap: {e}"); std::process::exit(1) });
    let map: PortMap = Arc::new(Mutex::new(HashMap::new()));
    let mut device = CatchAll::new(base, map.clone());
    ...
    eprintln!("smoltcp2socks_ca up on {tap_name} ip={ip_str} FIXED_PORT={FIXED_PORT} backlog={bl} (真 catch-all)");
