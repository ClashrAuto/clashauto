...
- **macOS — two product lines ship side by side, distinguished only by a `-qt` in the asset name:**
  ...
  - ★ **The `-qt` marker is a hard contract, not cosmetics.** Both DMGs live on the same release, and both updaters pick assets by extension/keyword. `UpdateChecker.isQtLine` (Swift) excludes it; `UpdateController::recommendedIndex` (Qt, macOS branch) prefers it. Drop or rename the marker and each line starts silently pushing users onto the other app.
