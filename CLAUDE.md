...
- **`ConfigBuilder`** — generates `full.yaml`: merges the base config, plugin DNS/TUN blocks, subscription proxies, proxy groups, and auto-generated region groups. `ensureFullConfig(tunEnabled)` is the entry point.
...
- **`MainWindow`** — the whole UI (frameless titlebar with custom drag handling, sidebar, status/subscriptions/settings/logs/vip/about pages, footer with TUN/proxy/core switches). Owns a `ClashService` and pointers to `CoreController`, `TrayController`, `SubscriptionStore`. All styling is inline via `appStyle()`.
- **`TrayController`** — system tray menu, traffic display, quick core/proxy/TUN toggles.
