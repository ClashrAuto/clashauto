...
- **Gone (2026-08):** the older **Qt Widgets** version — `src/main.cpp`, `src/MainWindow.*`, `src/TrafficChart.*`, plus the two `assets/chevron-*.svg` only it referenced. None of it ever compiled; the one thing still tying it in was `TrayController`'s `MainWindow*`, which the QML app always constructed with `nullptr`, so all three `if (m_window)` branches were dead at runtime. `TrayController` now takes just a parent and reopens the window purely via `openWindowRequested`. The CLI test subcommands (`--build-config`, …) lived in that `main.cpp` and were never in the shipped build.
...
There is **no unit-test framework** for the C++ side (the Rust crate has `cargo test`: 19 tests, run in CI). Verification = CI builds+packages green, then `validate/` checks the packaged artifacts. Headless self-test hooks worth knowing (all are env vars on the built binary, exit 0 = pass): `COAST_STATICDEPS_SELFTEST` (are the Qt plugins actually *in* this binary — TLS backend, QSQLITE, ico/png, and the `QtQuick`/`Controls`/`Layouts`/`ClashAuto` QML modules; under a static Qt a missing plugin produces no build error whatsoever, and it also catches botched deployment on a shared build), `COAST_RUSTSTACK_SELFTEST` (Rust C-ABI round trip), `COAST_SMOLGW_SELFTEST` (whole gateway data plane: synthetic frames → SYN-ACK → SOCKS CONNECT with the right per-device user), `COAST_NDP_RA_SELFTEST` (RA parsing — the only guard against the silent "dual-stack device leaks IPv6" failure), `COAST_TPROXY_SELFTEST` / `COAST_PF_SELFTEST` (kernel rule layers; need root), `COAST_ARPPARSE_SELFTEST` (system `arp` output → per-interface IP→MAC; all three platforms' formats, dispatched at runtime so Linux CI covers the Windows one too), `COAST_SOCKSUDPUSER_SELFTEST` (does the SOCKS5 **UDP** outbound declare its real source address in the ASSOCIATE request — the core's only handle for attributing a datagram to a device; declaring `0.0.0.0:0` fails silently), `COAST_NICEGRESS_SELFTEST` (the per-NIC listener generation: builds a throwaway device ledger, really runs `ensureFullConfig`, asserts on the product, and prints the `full.yaml` path), `COAST_TOPO_DUMP` (not a test — a live dump of what this machine's topology actually resolved to: primary NIC, per-NIC gateway + gateway MAC, the ARP table split per interface, cross-interface conflicts). ⚠️ The TPROXY/pf **data planes** have no end-to-end self-test — a known gap, not a solved problem.

★ **Where a hook's `if` goes in `main_qml.cpp` is decided by which `.cpp` defines it, not by what it sounds like.** Anything implemented in `src/net/RustStackSelfTest.cpp` must stay inside `#ifdef COAST_HAVE_RUST_STACK` (that file is only compiled under `if(WIN32)` + `COAST_RUST`, so on Linux/macOS the symbol doesn't exist) — that includes `COAST_GW_THROUGHPUT` and `COAST_GW_COASTCORE_SELFTEST`, whose names give no hint of Rust. Anything implemented in `BACKEND_SOURCES` (all three platforms) must go **outside** it. Getting this backwards costs more than a link error: an unregistered hook means setting its env var just **starts the GUI normally and never exits** — the test hangs instead of failing, which is far harder to diagnose (measured: 316 s still in `do_poll`, and only `/proc/<pid>/environ` revealed which hook it was). This bit the repo four times — `SELFROUTE`, `CRYPTO`, `ARPPARSE`, then in 2026-08 a whole batch (`RULE`/`PROXYCFG`/`CRYPTO`/`TUNSERVICE`/`OUTBOUND_PROBE`/`SELFROUTE` had been unreachable on Linux and macOS the entire time). Both CI legs now run every hook under a **30 s timeout** so the next occurrence is a red light with a message rather than a job that hangs to its 6-hour limit.

★ **On Windows a proxied device's identity rides *only* on the SOCKS username — and TCP and UDP carry it by different means.** The userspace stack terminates the device's flow and re-dials mihomo from `127.0.0.1`, so the real source IP is gone (Linux TPROXY and macOS pf are kernel redirections and keep it, which is why neither needs a username at all — `ConfigBuilder` deliberately emits **no** `users:` on those listeners and expresses identity with `SRC-IP-CIDR` instead). TCP gets the username from the SOCKS auth handshake. **UDP can't** — SOCKS5's UDP relay is a shared socket and datagrams carry no auth — so the client must declare its real `127.0.0.1:<port>` in the ASSOCIATE request's `DST.ADDR/DST.PORT` and the core keys `addr → user` off it (`clash` submodule: `listener/socks/udpuser.go`). Both halves are needed; either alone is a no-op, so neither needs a version gate. Get it wrong and nothing errors: `IN-USER` rules just stop matching for UDP and the device's QUIC traffic (YouTube) gets billed to 「本机」.
...
It downloads every platform's Coast artifact from the latest `ClashrAuto/clashauto` release, checks sha256, runs **structural** checks (Windows flat + **zero DLLs and zero subdirectories**, macOS `Coast.app` + `com.yuehongsun.coast` bundle id + helper, Linux flat `/opt/coast/coast` with no bundled `lib/ plugins/ qml/ qt.conf`), asserts `ldd` shows **no `libQt6`**, runs `COAST_STATICDEPS_SELFTEST` **in the clean container** (the only place a missing static plugin actually shows — a dev box can load it from elsewhere), and **actually runs the Linux build headless** under Xvfb — **twice, once per render backend** (default RHI/OpenGL, then `QT_QUICK_BACKEND=software`) — to confirm it launches on both paths and seeds its embedded config to `~/.local/share/Coast/config/`. Windows/macOS binaries can't execute in Linux Docker → structural only (real-machine run is the final word). See `validate/README.md`.
...
- **`TrayController`** — system tray menu, traffic display, quick core/proxy/TUN toggles, `notify()`. Reopening the main window goes through its `openWindowRequested` signal, which `main_qml.cpp` wires to the `QQuickWindow`.
...
- **Windows** — `Coast.exe`; installs to `%LOCALAPPDATA%\Coast`; portable zip is **flat and DLL-free** — just `Coast.exe` + `core.exe`, no subdirectories at all (Qt and the MSVC runtime are statically linked; see the static-Qt section below).
...
- **Self-contained CI** — no Clashr-Auto download/staging; the package is the exe + a bundled `core[.exe]`, nothing else (resources embedded in the exe; the "Bundle latest stable core" step downloads the latest **stable** `coast-<os>-<arch>` release asset from `ClashrAuto/clash`, same pick rules as `CoreRelease.h`, and fails the build if it can't).
- Windows CI = **MSVC (VS 17 2022)**; Linux/macOS = Ninja. ARM64 Windows is a cross-compile (`-DQT_HOST_PATH`, host Qt from aqt).

### ★ Qt is statically linked (since 2026-08) — the package is one executable

`aqtinstall` only ships **shared** Qt; the Qt Company never publishes static binaries. So a static Qt is **built from source in CI**: `.github/actions/qt-static/` (a composite action: restore cache → build on miss → save cache) with `build-posix.sh` / `build-windows.ps1`, warmed by `.github/workflows/qt-static.yml` (manual + weekly cron, purely to keep the cache from being evicted after 7 idle days). Five configs; first build is 40–110 min each, afterwards every push restores in seconds. `cache-hit=false` in a release job just means the cache aged out — re-run `qt-static.yml`, it is not a code problem.

- **Module set is exactly `qtbase + qtshadertools + qtdeclarative`.** No `qtsvg` (the repo uses no `.svg` at all any more — the two `chevron-*.svg` went out with the dead Widgets UI; the QML icons are all fonts) and no `qttools` (nothing left to deploy). If QML ever uses a real `.svg`, add `qtsvg` back **and** re-add the `svg` assertion in `src/StaticDepsSelfTest.cpp` — either half alone is wrong.
- **TLS backend per platform, chosen so no OpenSSL ships with the app**: Windows `-schannel -no-openssl` (the shared build already resolved to Schannel — verified `backends=cert-only,schannel`), macOS `-securetransport -no-openssl`, Linux `-openssl-runtime` (the plugin is static, it `dlopen`s `libssl.so.3`). ⚠️ That `dlopen` is why **`libssl3` can never appear in the ELF `NEEDED` table** and must stay hardcoded in the `.deb` `Depends` — the automated NEEDED→package reverse-lookup in CI will never flag it.
- **Windows also goes `/MT`** (`-static-runtime` for Qt, `CMAKE_MSVC_RUNTIME_LIBRARY` for the app, `RUSTFLAGS=-Ctarget-feature=+crt-static` for `coaststack`) so `vcruntime140`/`msvcp140*.dll` are gone too. Knock-on effect: the runner's own OpenSSL is `/MD` and would fail `CMakeLists`' real-link test → the three in-process protocols would **silently** fall back to mihomo. So the qt-static action also builds `openssl:x64-windows-static` via vcpkg into `<qt-prefix>/_openssl` (same cache entry).
- **Linux "static" stops at Qt.** glibc / xcb / GL / fontconfig cross the kernel-and-driver boundary and must stay dynamic; glibc in particular would break NSS *and* the `dlopen` above. Only `-static-libgcc -static-libstdc++` are added. The `.deb` still has a `Depends` list — but Qt no longer occupies any of it, and the dev packages it needs at **link** time are installed by the Linux job itself (**not** left to the qt-static action: on a cache hit that action skips its `apt` step entirely, and you'd get "first build fine, second build a wall of `undefined reference to xcb_*`" from identical source).
- ★ **On Windows, Ninja must actually be on `PATH` — Qt's configure falls back *silently*.** `QtProcessConfigureArgs.cmake` does `find_program(ninja ninja)`; if it misses, on Windows with `cl.exe` it picks **`NMake Makefiles`**, which is strictly single-threaded, and `cmake --build --parallel` cannot help. Measured cost of getting this wrong: Linux 31–41 min and macOS *universal* 48 min for the same module set, while both Windows jobs ran past **GitHub's 6-hour job ceiling** and were killed — with job logs needing repo-admin rights, so the burn bought zero information. (The first version's bug was `choco install ninja` without refreshing `PATH` in the current process, so the child `configure.bat` still couldn't see it.) There are now three guards: resolve Ninja explicitly (VS's bundled copy → choco, then prepend to `PATH`) and hard-fail if still missing; pass `-cmake-generator Ninja`; and after configure, **read `CMAKE_GENERATOR` back out of `CMakeCache.txt` and fail unless it is `Ninja`** — that last one turns a 6-hour timeout into a 5-minute red light. Per-module elapsed times are emitted as `::warning::` annotations because **annotations are publicly readable while logs are not**.
- **CMake auto-detects** which Qt it got by looking at `Qt6::Core`'s target `TYPE` (not `QT6_IS_SHARED_LIBS_BUILD`, which is undefined in some layouts — and `if(NOT <undefined>)` is true, which would send a shared build down the static path). Both kinds still build, so local dev against a shared Qt is unchanged.
- **The one guard that matters**: `COAST_STATICDEPS_SELFTEST=1` (`src/StaticDepsSelfTest.cpp`, run in all three CI legs and in `validate/`). Under a static Qt, plugins are pulled in at **link** time by `qt_import_qml_plugins()` plus Qt's default plugin import; missing one produces **no compile or link error at all** — just a silently missing feature (TLS backend → "network is down", QSQLITE → history silently not recorded, QML plugins → an empty window). Note `CMakeLists` deliberately does *not* narrow plugins with `qt_import_plugins(... INCLUDE_BY_TYPE platforms ...)`: that would drop `qoffscreen`, which every headless CI self-test runs on.
