...
- **Module set is exactly `qtbase + qtshadertools + qtdeclarative`.** No `qtsvg` (the two `chevron-*.svg` in `resources.qrc` are referenced *only* from the dead `MainWindow.cpp`; the QML UI's icons are all fonts) and no `qttools` (nothing left to deploy). If QML ever uses a real `.svg`, add `qtsvg` back **and** re-add the `svg` assertion in `src/StaticDepsSelfTest.cpp` — either half alone is wrong.
